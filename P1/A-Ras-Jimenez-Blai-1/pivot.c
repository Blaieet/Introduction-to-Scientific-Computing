 ...
 * DNI: 46980893N
 * Created on November 6, 2016, 8:28 PM
 */
...
void matrix_writee(int n, double**u, FILE* fout) {  // CORRECCIÓ
...
/*Funcio que dur a terme el moviment de files*/
void pivotatge(int n, double** A, int col) {
   
    double max;
    int i,pos;
    
    max = fabs(A[col][col]); /*Es troba el maxim i la columna afectada*/
    pos = col;
    
    for (i = col+1; i <n; i++) { /*Si per aquesta columna*/
        
        if (fabs(A[i][col]) > max) { /*Trobem un element mes gran que el primer*/
            pos = i; /*Cambia'l*/
            max = fabs(A[i][col]); /*Estableixo*/
        } 
    }
    
    double aSota; /*Auxiliar*/
    
    if (pos != col) { /*Moviment de fila*/
        for (i = 0; i < n+1; i++) {
            aSota=A[col][i]; /*Fila que s'enva a sota*/
            A[col][i]=A[pos][i]; /*Fila que s'enva a dalt*/
            A[pos][i] = aSota; /*Recuperem la primera fila*/
        }
    }
}

/*Funcio que duu a terme el proces de Gauss-Jordan amb pivotatge maximal i donada una tolerancia. Resol tambe el sistema amb resoltrisup*/
int gausspivot(int n, double **A, double *v, double tol,double* x) {
    
    int i,j,k,index;
    double multiplicador;
    
    for (i=0; i< n; i++) {
        pivotatge(n,A,i); /*Mira si fa falta un intercanvi per cada columna*/
        for (j=i+1; j<n; j++) {
            for (k=0; k <n; k++) { /*Mirem si compleix la tolerancia*/
                if (fabs(A[k][k]) <= tol) {
...
int main(int argc, char** argv) {
    ...
    fout = fopen("Uxy3.res","w"); /*Fitxer d'escriptura de dades*/
    fin = fopen("Uxy3.dad","r"); /*Fitxer de lectura de dades*/
